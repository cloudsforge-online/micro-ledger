 ...
 *      rollback of the image is not a rollback of the database. For the ledger that would mean an
 *      image rollback silently leaving the newer constraint triggers in place, or removing them.
 *
...
try {
  ...
  // Exit non-zero and loudly. The deploy must stop here: a ledger started against a schema its
  // migrator could not reach is a ledger that may be missing the triggers that are its entire
  // safety argument.
