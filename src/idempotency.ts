 ...
 * the best code in the existing estate and the ledger inherits its behaviour rather than inventing
 * a second one. What it gets right, and what is preserved here in full:
 *
 ...
 *
 * The deferred balancing trigger fires at COMMIT, *after* the response is stored. That ordering is
 * load-bearing and correct: if the entry does not balance, the COMMIT fails and takes the claim
 * row with it, so an unbalanced entry never leaves a key behind that would make its retry a
 * no-op.
 */
 ...
 *
 * Exported because `journal_entries.idempotency_key` stores this same namespaced value: the unique
 * constraint there and the primary key here must describe the same thing, or the two could
...
export interface IdempotencyInput<T> {
  ...
  /**
   * The work. Returns the response to store and, when the work created one, the entry it created —
   * so the claim row points at the entry and an operator can join a key to its postings.
   */
  readonly run: (tx: Tx, storedKey: string) => Promise<{ response: T; entryId: string | null }>
}
...
export async function withIdempotency<T>(sql: Db, input: IdempotencyInput<T>): Promise<IdempotentOutcome<T>> {
  ...
  const outcome = await sql.begin(async (tx) => {
    ...
    const { response, entryId } = await input.run(tx, key)
 ...
 *
 * A claim row whose entry still exists is kept regardless of age. The row is the only link between
 * a caller's key and the postings it produced, and losing it turns "did my retry post twice"
