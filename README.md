...
> (`src/server.ts`). A user token presented to `GET /accounts/:subject/balances` gets the same
...
> projection (`src/migrations.ts`), rebuilt nightly by replaying the journal into
...
constraint trigger rather than by a handler (`src/migrations.ts` on `journal_entries`,
`src/migrations.ts` on `postings`, function at `src/migrations.ts`).
...
* **An entry is several INSERTs.** It is unbalanced between the first posting and the last, so an
  ...
  unbalanced entry exists (`src/migrations.ts`).
...
* **Per asset, not per entry.** A conversion debits EMBER and credits Shards in one atomic entry
  ...
  entry balance and a correct one fail (`src/migrations.ts`).
...
nothing behind (`src/entries.test.ts`). A second test appends a posting to an
already-committed entry **in a later transaction** and is refused too, which is why the trigger is
on `postings` as well (`src/entries.test.ts`). The file says why in its header: a suite that
only ever posts through `postEntry` would prove that `validateEntryRequest` works, not that the
ledger is safe (`src/entries.test.ts`).
...
| --- | --- | --- | --- | --- | --- |
| `GET` | `/livez` | **anyone** | — | — | liveness; makes no `authorise()` call (`src/server.ts`) |
| `GET` | `/readyz` | **anyone** | — | — | 200 ready / 503 not; makes no `authorise()` call (`src/server.ts`) |
| `GET` | `/metrics` | **anyone** | — | — | Prometheus text; makes no `authorise()` call (`src/server.ts`) — see Known gaps |
| `POST` | `/entries` | service | `ledger:post` | **required**, in the body | posts a journal entry; 201 fresh, **200 on a replay** so a caller can tell whether its retry did the work (`src/server.ts`, status) |
| `GET` | `/entries` | service | `ledger:read` | — | cursor page, filterable by `originatingService`, `correlationId`, `kind` (`src/server.ts`) |
| `GET` | `/entries/:id` | service | `ledger:read` | — | one entry with its postings; 404 if absent (`src/server.ts`) |
| `POST` | `/entries/:id/reverse` | service | `ledger:post` | **required**, in the body | writes a **new** reversing entry with `reverses_entry_id` set; never edits the original (`src/server.ts`) |
| `POST` | `/reservations` | service | `ledger:reserve` | **required**, in the body | moves value to a reserved account; the reservation **is** the entry, so `reservationId` is the entry id (`src/server.ts`, note) |
| `POST` | `/reservations/:id/release` | service | `ledger:reserve` | **required**, in the body | releases a reservation; a second release is 409 `already_released` (`src/server.ts`) |
| `GET` | `/accounts/:subject/balances` | service | `ledger:read` | — | balances for one subject; the subject is percent-decoded because it is `user:<uuid>` (`src/server.ts`) |
| `GET` | `/trial-balance` | service | `ledger:read` | — | Σ\|debits − credits\| across every asset. **200 even when it is not zero** — the caller asked what the trial balance is and this is the answer; a 500 would deny a monitor the number it exists to read (`src/server.ts`, reasoning) |
| `GET` | `/reconciliation` | service | `ledger:read` | — | the latest run per asset plus the current freezes (`src/server.ts`) |

**Three routes make no `authorise()` call**: `/livez`, `/readyz` and `/metrics`
(`src/server.ts`). A client that sends a token to them is not refused — they
...
(`src/server.ts` for `/entries` for the others). There is **no**
...
only reads (`src/server.ts`).

### Status codes

Grouped by what the caller should do (`src/server.ts`): **400** the request could not be a
...
every service in the estate out because identity is having a bad minute (`src/server.ts`).
...
with an error that says to send a string (`src/server.ts`).
...
one. **The lease key names the contended resource, not the row** (`src/jobs.ts`).

| Job | Lease key | Cadence | What two replicas do |
| --- | --- | --- | --- |
| `outbox.relay` | `stream` | 1s | one claims the stream; the other finds nothing. Two relays would deliver one batch to one subscriber twice (`src/jobs.ts`) |
| `ledger.reconcile` | `asset:<code>` | 15min | keyed per asset because assets genuinely parallelise. Keyed on the run id instead would let two runs of one asset race to set and clear its freeze, and **a freeze cleared by a stale run is a withdrawal path reopened against drift that still exists** (`src/jobs.ts`, reasoning) |
| `ledger.balances.rebuild` | `global` | 24h | `balances_shadow` is one table for the whole chart; two rebuilds would interleave DELETE and INSERT and report mismatches that are artefacts of each other (`src/jobs.ts`) |
| `ledger.idempotency.reap` | `global` | 24h | nothing breaks, but two reapers is two long DELETEs competing for row locks at the head of every posting request (`src/jobs.ts`) |
...
such a liability is withdrawable (`src/jobs.ts`).
...
attempt budget is a ledger that has stopped being checked (`src/jobs.ts`).
...
`src/migrations.ts` states the design plainly: 04-domain-model.md §2.2 requires these
...
| --- | --- | --- |
| `journal_entries_balanced` / `postings_balanced` (deferred constraint triggers) | an unbalanced entry, and an entry with no postings at all | it holds at `COMMIT` against a caller holding a database connection, which a handler does not. The `postings` copy exists so a posting appended in a *later* transaction cannot unbalance a committed entry (`src/migrations.ts`) |
| `ledger_refuse_mutation` triggers **+** `revoke update, delete, truncate` | any UPDATE or DELETE of a posting or an entry | **two mechanisms because they bind different people.** The REVOKE binds every non-owner role; the trigger binds *everyone*, including the owner and a superuser in psql — and the owner is the account most likely to be used for a well-meant quick fix. An audit trail that shows only the fix and not the mistake cannot answer "what did we think was true on the 3rd" (`src/migrations.ts`, reasoning) |
| `ledger_assert_no_overdraft` (immediate, **AFTER** insert or update) | a liability going negative without `overdraft_allowed` | immediate, not deferred, because paying out value a user never had must fail at the posting and name the account. **AFTER rather than BEFORE is not stylistic**: the projection is written `INSERT … ON CONFLICT DO UPDATE` where the inserted `amount` is the *delta*, and Postgres fires BEFORE-INSERT row triggers before it detects the conflict — a BEFORE trigger would see the raw delta, refuse every debit, and *appear* to work because a debit against an account with no balance row fails for the right reason by accident (`src/migrations.ts`, reasoning) |
| `journal_entries_idempotency_key_uniq` | a second entry under one key, for ever | the uniqueness is the invariant; `idempotency_keys` is only what lets a duplicate be *replayed* rather than refused. Claimed in the same transaction as the postings, so a stored response can never disagree with what committed (`src/migrations.ts`, `src/migrations.ts`) |
| `journal_entries_no_self_reversal_chk` | `reverses_entry_id = id` | a correction is a **new** entry; an entry that reverses itself is not representable (`src/migrations.ts`) |
| `journal_entries_kind_chk` | an entry kind nobody enumerated | an unenumerated kind is a category that exists in the data and in no report (`src/migrations.ts`) |
| `postings_amount_positive_chk` | amount ≤ 0 | direction carries the sign. A signed amount plus a direction is two ways to say one thing and they drift (`src/migrations.ts`) |
| `postings_entry_sequence_uniq` | two postings at one sequence in one entry | replay order is the journal's order; a duplicate sequence makes a rebuild non-deterministic (`src/migrations.ts`) |
| `accounts_key_uniq` on `(subject, asset_code, purpose)` | a second account for one key | the account key *is* those three columns; a duplicate splits one balance in two (`src/migrations.ts`) |
...
overdraft trigger exists to refuse (`src/migrations.ts`).
...
still a bug but one that owes the user rather than the reverse (`src/migrations.ts`).
...
| --- | --- | --- |
| `PORT` | `4000` | must be an integer 1–65535 or boot fails (`src/env.ts`) |
| `NODE_ENV` | `development` | labelling only (`src/env.ts`) |
| `LOG_LEVEL` | `info` | anything outside `debug\|info\|warn\|error` refuses to start (`src/env.ts`) |
| `CLOUDSFORGE_TAG` | `dev` | the version reported in logs and `/livez` is wrong (`src/env.ts`) |
| `LEDGER_DATABASE_URL` | — | **required**; the service refuses to start (`src/env.ts`). Rule 1 — CI greps for any second connection-string variable, so adding one fails the build rather than review (`src/env.ts`) |
| `LEDGER_DATABASE_POOL_MAX` | `10` | integer 1–500; too low serialises postings, too high exhausts Postgres connections (`src/env.ts`) |
| `IDENTITY_JWKS_URL` | — | **required**. Unreachable at runtime → every domain route answers 503, not 401 (`src/env.ts`, `src/server.ts`) |
| `IDENTITY_ISSUER` | — | **required**. Wrong value → every token fails verification and the whole surface is 401 (`src/env.ts`) |
...
| `INSTANCE_ID` | hostname | names this replica in `jobs.locked_by`; wrong only makes a stuck lease harder to attribute (`src/env.ts`) |
...
| `LEDGER_RECONCILE_NETWORK` | `testnet` | must be `mainnet` or `testnet`; recorded on every run row (`src/env.ts`) |
...
| `LEDGER_IDEMPOTENCY_TTL_DAYS` | `30` | **expiring a key early means the next replay does the work a second time**, so this must outlive every caller's retry horizon rather than be as short as the table would like (`src/env.ts`, reasoning) |
...
| `micro-identity` | its JWKS document at `IDENTITY_JWKS_URL`, fetched by `@cloudsforge/auth`'s `Verifier` (`src/index.ts`) | **fail closed for new tokens, but not fatal.** Every domain route answers **503 `verifier_unavailable`**, never 401 (`src/server.ts`). The readiness probe is deliberately **soft** (`src/index.ts`): marking it hard would remove every service in the estate from its load balancer on one identity blip, which is a cascade, not a safety measure |
| whatever rows are in `event_subscriptions` | signed HMAC event deliveries from the outbox relay (`src/outbox.ts`) | **fail open, per subscriber.** A failed delivery records `last_error` and leaves `delivered_at` null; the batch continues and the job succeeds, because one dead subscriber must not stop the stream. The undelivered row is the durable record (`src/outbox.ts`) |
...
## The four tests CI still does not run

CI is **green**, and the badge is honest: 195 tests, 191 passing, 0 failing. But four of them do
not execute there, and that is worth one paragraph because they are the most valuable four in the
repository.

They are the `LIVE` tier in `src/chainbacking.test.ts`: `micro-indexer`'s real `createServer`,
`rpcCustodyObserver` and `RpcPool`, against a deterministic JSON-RPC node on a real port, imported
across the checkout. With them, the chain of custody from `eth_getBalance` to a row in
`reconciliation_runs` contains **no test double at all**. They stand down in CI because CI checks
out one repository and they need two — a `micro-indexer` checkout beside this one and a second
Postgres.

This used to fail the build rather than merely skip. The shared rule greps test output for a
skipped database suite — correctly, because a suite whose database tests silently skip is the worst
possible green — but its pattern matched **any** service's DSN name, so these cases naming
`INDEXER_TEST_DATABASE_URL` looked identical to this service's own database being absent. That is
now fixed in `micro-org`: a skip naming another service's variable is a **notice**, a skip naming
this service's own is still fatal, and both at once is still fatal. The distinction is reported
...
where the first token had already been refused with a real 401 by the real indexer.
...
* **The loop is closed. What remains is a deploy step and a chain that has not launched.** This
  ...
  **The grant landed, and then the credential behind it expired every ten minutes.**
  ...
  > `LEDGER_SERVICE_TOKEN` was a **600-second** token (`identity/src/tokens.ts`) read once at
...
* **No OpenAPI description.** 11-data-and-contract-strategy.md names one as the mechanism for
...
* **`BASELINE_VERSION = 0`** (`src/migrations.ts`): there is nothing to adopt. Moving value out
