...
# the ledger's migrations create the constraint triggers that are the service's entire safety
# argument, and a service that could create them at boot is a service that could start without
# them. `index.ts` asserts the schema version and refuses to serve below it.
